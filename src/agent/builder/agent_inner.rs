...
//! (dirge-4y4l stage 11c): `build_agent_inner` wires the rig `Agent`
//! (preamble + tool registry + provider model) under the wide
//! dependency-injection signature, plus the plugin `hookify` wrapper it
//! applies to every tool. Helpers it composes (preamble assembly,
//! `build_loop_tools`, `build_session_search_tool`) come from the
//! sibling modules via the parent’s re-exports.
...
use super::{
    ...
    build_session_search_tool, model_steering_fragment,
};
